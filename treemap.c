...
Pair * upperBound(TreeMap * tree, void* key)
{
    ...
    while (1)
    {
        ...
        //si la key del current es mayor a la que estoy buscando
    ...
    if ((tree->lower_than(tree->current->pair->key , key) == 1 || tree->lower_than(key , tree->current->pair->key) == 1) && ub_node != NULL)
    {
        return ub_node->pair;
    }
